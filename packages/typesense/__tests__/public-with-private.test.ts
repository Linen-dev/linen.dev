...
describe('public community with one private channel', () => {
  ...
  describe('using community token', () => {
    ...
    test('querying for messages from public channel should work', async () => {
      const data = await fetcher(searchSettings.apiKey, 'lorem');
  ...
  describe('using user token', () => {
    ...
    test('user1 token should exist', async () => {
      ...
      expect(userSettings.scope).toBe('public');
